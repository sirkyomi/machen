...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-settings-tab="capture"]').click();await page.getByRole('button',{name:'Change shortcut',exact:true}).click();await page.keyboard.press('Control+Alt+K');await page.waitForFunction(()=>window.api.call('state').then(s=>s.settings.shortcut==='CommandOrControl+Alt+K'));await page.locator('[data-settings-tab="updates"]').click();
