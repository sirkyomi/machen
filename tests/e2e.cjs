...
try{
 ...
 const page=await app.firstWindow();await page.getByRole('heading',{name:'Ein ruhiger Ort für deine Aufgaben.'}).waitFor();await page.screenshot({path:'test-results/onboarding.png'});
 await page.getByRole('button',{name:'Machen einrichten',exact:true}).click();await page.getByRole('heading',{name:'Wie soll Machen aussehen?'}).waitFor();
 await page.getByRole('button',{name:'Dunkel',exact:true}).click();assert.equal(await page.locator('html').getAttribute('data-theme'),'dark');
 await page.getByRole('button',{name:'Weiter',exact:true}).click();await page.getByRole('heading',{name:'Wähle deinen Akzent.'}).waitFor();
 await page.getByRole('button',{name:'Smaragd',exact:true}).click();assert.equal(await page.locator('html').getAttribute('data-palette'),'emerald');
 await page.getByRole('button',{name:'Weiter',exact:true}).click();await page.getByRole('button',{name:'Ablageort wählen'}).waitFor();
 ...
 await reopened.getByRole('button',{name:'Einstellungen',exact:true}).click();await reopened.locator('[data-settings-tab="app"]').click();
 await reopened.getByRole('button',{name:'Machen zurücksetzen',exact:true}).click();await reopened.getByRole('dialog').getByRole('button',{name:'Jetzt zurücksetzen',exact:true}).click();
 await reopened.getByRole('heading',{name:'Ein ruhiger Ort für deine Aufgaben.'}).waitFor();assert.equal((await reopened.evaluate(()=>window.api.call('state'))).configured,false);assert.equal(fs.existsSync(path.join(second,'todo.txt')),true);
