...
try{
 ...
 await page.getByText('Angebot für Studio Nord',{exact:true}).click();await page.locator('#notes').fill('Rückmeldung aus der E-Mail vom Montag.');await page.getByRole('button',{name:'Speichern',exact:true}).click();await page.locator('.task-dialog').waitFor({state:'detached'});
 ...
 const quick=app.windows().find(w=>w.url().includes('quick=1'));await quick.getByRole('textbox',{name:'Neue Aufgabe'}).fill('Zahnarzttermin vereinbaren');await quick.getByRole('button',{name:'Erfassen',exact:true}).click();
 ...
 await page.getByRole('button',{name:'Einstellungen',exact:true}).click();await page.locator('[data-settings-tab="app"]').click();
 ...
 await app.close();app=await electron.launch({args:['.'],env:{...process.env,MACHEN_TEST_HOME:home}});
  await app.firstWindow();let reopened;for(let i=0;i<100&&!reopened;i++){reopened=app.windows().find(w=>w.url().includes('index.html')&&!w.url().includes('quick=1')&&!w.url().includes('pinned=1'));if(!reopened)await new Promise(resolve=>setTimeout(resolve,50));}await reopened.getByRole('heading',{name:'Heute',exact:true}).waitFor();
